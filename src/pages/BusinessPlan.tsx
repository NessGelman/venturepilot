      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <p style={{ color: '#8798b0', marginTop: 6, fontSize: 15 }}>
            Comprehensive strategic roadmap for your venture.
          </p>
        </div>
        <button
          onClick={() => {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
            {milestones.map((m, idx) => (
              <button
                key={idx}
                onClick={() =>
                  setMilestones(milestones.map((x, i) => (i === idx ? { ...x, done: !x.done } : x)))
                }
                style={{
                  display: 'flex',
        ...
        <Card>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              ...
              >
                <h4 style={{ color: '#f0f4ff', fontSize: 13, fontWeight: 700, marginBottom: 5 }}>
                  {item.t}
                </h4>
