...
import {
  ...
  BarChart2, Globe, ArrowRight, Building2, Edit2, FileDown
} from 'lucide-react';
...
export default function PitchDeck() {
  ...
  // Edit mode: keyed by slide index → override subtitle
  const [editingIdx, setEditingIdx] = useState<number | null>(null);
  const [overrides, setOverrides] = useState<Record<number, string>>({});
  const [editDraft, setEditDraft] = useState('');
  const slide = slides[current];
  const color = SLIDE_COLORS[slide.type] || '#3b82f6';
  const hasOverride = overrides[current] !== undefined;
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (editingIdx !== null) return; // don't navigate while editing
  ...
  const startEdit = () => {
    setEditDraft(overrides[current] ?? slide.subtitle ?? '');
    setEditingIdx(current);
  };

  const commitEdit = () => {
    setOverrides(prev => ({ ...prev, [current]: editDraft }));
    setEditingIdx(null);
  };
...
${slides.map((s, i) => `<div class="slide"><div class="meta">Slide ${s.id} · ${s.type}</div><h1>${s.title}</h1>${(overrides[i] ?? s.subtitle) ? `<h2>${overrides[i] ?? s.subtitle}</h2>` : ''}<div class="notes"><strong>Speaker Notes:</strong><br>${s.notes}</div></div>`).join('')}
</body></html>`;
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      // Build a standalone HTML document for the PDF
      const ideaName = state?.idea || 'VenturePilot';
      const container = document.createElement('div');
      container.style.cssText = 'font-family:-apple-system,sans-serif;background:#04060d;color:#e2e8f0;padding:24px;';
      container.innerHTML = `
        <h1 style="font-size:28px;margin-bottom:8px;color:#93c5fd">${ideaName} Pitch Deck</h1>
        <p style="color:#64748b;margin-bottom:24px;font-size:12px">Generated by VenturePilot · ${new Date().toLocaleDateString()}</p>
        ${slides.map((s, i) => `
          <div style="border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:24px;margin-bottom:20px;background:rgba(255,255,255,0.02);page-break-inside:avoid">
            <div style="font-size:10px;color:#64748b;margin-bottom:4px">Slide ${s.id} · ${s.type.toUpperCase()}</div>
            <h2 style="font-size:22px;color:#fff;margin-bottom:8px">${s.title}</h2>
            ${(overrides[i] ?? s.subtitle) ? `<p style="font-size:13px;color:#93c5fd;margin-bottom:12px">${overrides[i] ?? s.subtitle}</p>` : ''}
            <div style="margin-top:12px;padding:12px;background:rgba(59,130,246,0.08);border-left:3px solid #3b82f6;border-radius:4px;font-size:11px;color:#94a3b8;line-height:1.6">${s.notes}</div>
          </div>`).join('')}
      `;
      document.body.appendChild(container);
      await html2pdf().set({
        margin: 0,
        filename: `${ideaName.replace(/\s+/g, '-')}-pitch-deck.pdf`,
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" icon={FileDown} onClick={exportPDF} disabled={exportingPdf}>
          ...
          <div className="flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-y-auto lg:max-h-[600px] custom-scrollbar pb-2">
            {slides.map((s, i) => (
              <button key={s.id} onClick={() => setCurrent(i)}
                ...
                <div className="text-[9px] font-bold mb-1 flex items-center gap-1 justify-between" style={{ color: SLIDE_COLORS[s.type] }}>
                  <span><span className="opacity-60">{s.id}.</span> {s.type.toUpperCase()}</span>
                  {overrides[i] !== undefined && (
                    <span className="text-[8px] px-1 py-0.5 rounded bg-[rgba(245,158,11,0.2)] text-[var(--amber)] font-bold">Edited</span>
                  )}
                </div>
                ...
                <div className="flex-1 flex flex-col justify-center">
                  <div className="flex items-start gap-3 mb-3">
                    <motion.h1
                      ...
                      className="text-3xl lg:text-4xl font-black leading-tight flex-1"
                    ...
                    <div className="flex items-center gap-1.5 shrink-0 mt-1">
                      {hasOverride && (
                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-[rgba(245,158,11,0.2)] text-[var(--amber)] font-bold">Edited</span>
                      )}
                      <button
                        onClick={editingIdx === current ? commitEdit : startEdit}
                        className="flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-[var(--radius-md)] border transition-all"
                        style={{
                          background: editingIdx === current ? color + '20' : 'transparent',
                          borderColor: editingIdx === current ? color : 'var(--border)',
                          color: editingIdx === current ? color : 'var(--text-muted)',
                        }}
                      >
                        <Edit2 size={10} />
                        {editingIdx === current ? 'Done' : 'Edit'}
                      </button>
                      {hasOverride && (
                        <button onClick={clearOverride} className="text-[9px] font-bold text-[var(--text-muted)] hover:text-[var(--red)] px-1.5 py-1 rounded border border-[var(--border)] transition-colors">
                          Revert
                        </button>
                      )}
                    </div>
                  ...
                  {editingIdx === current ? (
                    <textarea
                      ...
                      onBlur={commitEdit}
                      rows={4}
                      className="w-full max-w-2xl bg-[rgba(255,255,255,0.05)] border border-[var(--accent)] rounded-[var(--radius-md)] px-3 py-2.5 text-sm text-[var(--text-secondary)] leading-relaxed resize-none focus:outline-none mb-3"
                      placeholder="Edit slide subtitle / key message…"
                    />
                  ) : (
                    (overrides[current] ?? slide.subtitle) && (
                      <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                        className="text-sm lg:text-base text-[var(--text-secondary)] leading-relaxed max-w-2xl mb-3">
                        {overrides[current] ?? slide.subtitle}
                      </motion.p>
                    )
                  )}
