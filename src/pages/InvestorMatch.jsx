...
export default function InvestorMatch() {
  ...
  const filtered = useMemo(
    () =>
      investors.filter((inv) => {
        ...
        const matchQ =
          !q ||
          [inv.name, inv.focus, inv.stage, inv.contact, inv.link, inv.note].some((f) =>
  ...
  const addInvestor = (e) => {
    e.preventDefault();
    if (!newInv.name || !newInv.focus || !newInv.stage) {
      setStatus('Name, focus, and stage required.');
      return;
    }
    setInvestors((prev) => [
      { ...newInv, link: newInv.link || 'n/a', next: newInv.next || '2026-03-30' },
      ...prev,
    ]);
  ...
  const updateStatus = (idx, contact) =>
    setInvestors((prev) => prev.map((inv, i) => (i === idx ? { ...inv, contact } : inv)));
  const updateNext = (idx, date) =>
    setInvestors((prev) => prev.map((inv, i) => (i === idx ? { ...inv, next: date } : inv)));

  const counts = investors.reduce((acc, inv) => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      >
        <div>
          ...
          <p style={{ color: '#8798b0', marginTop: 6, fontSize: 15 }}>
            Track outreach, follow-ups, and warm intros.
          </p>
        </div>
      ...
      <Card>
        ...
        <p style={{ color: '#c7d2f0', fontSize: 14, lineHeight: 1.6 }}>
          Space: <strong>{industry || 'Set industry in sidebar'}</strong>. Problem solved:{' '}
          <strong>{problem || 'Add the pain point in sidebar'}</strong>.
        </p>
      </Card>
      ...
      <Card>
        ...
        >
          {[
            ...
            <input
              key={f.key}
              value={newInv[f.key]}
              onChange={(e) => setNewInv({ ...newInv, [f.key]: e.target.value })}
          ...
          <input
            type="date"
            value={newInv.next}
            onChange={(e) => setNewInv({ ...newInv, next: e.target.value })}
            style={inputSt}
          />
          <select
            value={newInv.contact}
            onChange={(e) => setNewInv({ ...newInv, contact: e.target.value })}
            style={inputSt}
          >
            {['Active', 'Interested', 'Dormant'].map((o) => (
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            >
              {['All', 'Active', 'Interested', 'Dormant'].map((o) => (
                <option key={o} value={o} style={{ color: '#0f172a' }}>
                  {o}
                </option>
              ))}
                ...
                <tr key={i} style={{ background: 'rgba(255,255,255,0.02)' }}>
                  <td style={{ padding: '14px', borderRadius: '10px 0 0 10px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        <p style={{ color: '#f0f4ff', fontWeight: 700, fontSize: 13 }}>
                          {inv.name}
                        </p>
                  ...
                  <td
                    style={{ paddingRight: 14, borderRadius: '0 10px 10px 0', textAlign: 'right' }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6 }}>
                      {[Mail, Linkedin, Send, ExternalLink].map((Icon, idx) => (
                        <button
                          key={idx}
                          onClick={idx === 2 ? () => copyTemplate(inv) : undefined}
                          style={{
                            width: 30,
                            height: 30,
                            borderRadius: 7,
        ...
        {status && (
          <p style={{ color: '#10b981', fontSize: 12, fontWeight: 700, marginTop: 12 }}>{status}</p>
        )}
      </Card>
    </div>
  );
