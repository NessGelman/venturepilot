...
export default function InputSidebar({ isOpen, setIsOpen }) {
  ...
  const analyzeRepo = async () => {
    ...
    if (!match) {
      addToast('Enter a valid GitHub repo URL.');
      return;
    }
    ...
    setProblem(
      (prev) =>
        prev || lines.find((l) => /problem|pain|why/i.test(l)) || `Core pain from ${repoName}.`,
    );
    setIndustry(
      (prev) => prev || lines.find((l) => /saas|ai|ml|fintech|health|infra/i.test(l)) || 'Software',
    );
      ...
      >
        ...
        >
            ...
            <button
              onClick={() => {
                if (presetName.trim()) {
                  ...
                  setPresetName('');
                  addToast(`Saved: ${presetName.trim()}`);
                }
              }}
              style={{
                display: 'flex',
          ...
          <div style={{ ...row2, marginTop: 8 }}>
            ...
            >
              ...
              {presets.map((p) => (
                <option key={p.name} value={p.name} style={{ color: '#0f172a' }}>
                  {p.name}
                </option>
              ))}
            </select>
              ...
              <button
                onClick={() => {
                  if (selectedPreset) {
                    ...
                    addToast(`Applied: ${selectedPreset}`);
                  }
                }}
                style={{
                  flex: 1,
              ...
              <button
                onClick={() => {
                  if (selectedPreset) {
                    ...
                    addToast(`Deleted: ${selectedPreset}`);
                    setSelectedPreset('');
                  }
                }}
                style={{
        ...
        >
          ...
          <p style={{ fontSize: 11, color: '#8798b0', lineHeight: 1.5 }}>
            Updating values recalculates runway, efficiency, and readiness across all modules in
            real time.
          </p>
        </div>
